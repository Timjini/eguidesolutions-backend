const http = require("http");
const express = require("express");
const app = express();
const server = http.createServer(app);
const io = require("socket.io")(server);
const cors = require("cors");
const verifyToken = require("./src/auth/authMiddleware");
const usersRoutes = require("./src/api/users/usersRoutes");
const channelsRoutes = require("./src/api/channels/channelsRoutes");
const agenciesRoutes = require("./src/api/agencies/agenciesRoutes");
const toursRoutes = require("./src/api/v1/tours/touristToursRoutes");
const adminRoutes = require("./src/api/admin/adminRoutes");
const touristToursRoutes = require("./src/api/v1/tours/touristToursRoutes");
const authRoutes = require("./src/api/v1/auth/authRoutes");
const favoriteRoutes = require("./src/api/v1/favorite/favoriteRoutes");
const touristChannelRoutes = require("./src/api/v1/channels/touristChannelsRoutes");
const agenciesRoutesV2 = require("./src/api/v2/agencies/agencyRoutes");
const toursRoutesV2 = require("./src/api/v2/agencies/toursRoutes");
const mainRoutes = require("./src/api/v1/main/mainRoutes");
const { RtcTokenBuilder, RtcRole } = require("agora-access-token");
const path = require("path");
const { upload, uploadToS3, getObjectFromS3 } = require("./src/utils/fileUploader");
const userProfileRoutesAdminPanel = require("./src/api/v2/userProfileRoutes");
const packagesRoutes = require("./src/api/v2/agencies/packagesRoutes");
const subscriptionsRoutes = require("./src/api/v2/agencies/subscriptionsRoutes");
const paymentsRoutes = require("./src/api/v2/agencies/paymentsRoutes");
const initializeDailyJobs = require("./src/jobs/DailyJobs");
const guideRoutesV2 = require("./src/api/v2/guideRoutes");
const bookingRequestRoutes = require("./src/api/v1/bookingRequestRoutes");
const marokoEkspertRoutes = require("./src/api/v3/marokoEkspertRoutes");


// =================================================================================================
// ========================================CORS POLICY =============================================[]
// =================================================================================================

const allowedOrigins = [
  "https://admin-eguide.vercel.app",
  "https://admin.e-guidesolutions.com",
  "http://localhost:5173",
  "http://localhost:3000",
  "https://maroko-ekspert.vercel.app",
  "https://www.maroko-ekspert.pl",
  "https://maroko-ekspert.pl"
];

app.use(
  cors({
    origin: function (origin, callback) {
      if (!origin) return callback(null, true);
      if (allowedOrigins.includes(origin)) {
        return callback(null, true);
      } else {
        return callback(new Error('Not allowed by CORS'));
      }
    },
    credentials: true,
    methods: "GET,HEAD,PUT,PATCH,POST,DELETE,OPTIONS",
    allowedHeaders: "Content-Type,Authorization",
  })
);

// =================================================================================================
// ======================================== R2 BUCKET / FILE UPLOADS ==============================[]
// =================================================================================================

app.post("/upload", async (req, res) => {
  try {
    upload(req, res, async function (err) {
      if (err) {
        console.error("Error uploading file:", err);
        return res.status(500).json({ error: "Error uploading file." });
      }

      const file = req.file;
      const result = await uploadToS3(file);

      res.json(result);
    });
  } catch (err) {
    console.error("Error uploading file:", err);
    res.status(500).json({ error: "Error uploading file." });
  }
});
// =================================================================================================
// =========================================== APP's ROUTES =======================================[]
// =================================================================================================

app.use(upload);

app.set("views", path.join(__dirname, "views"));
app.use(express.static(path.join(__dirname, "public")));
app.set("view engine", "ejs");
app.use(express.json());
// app.use("/", (req, res) => {
//   // res.render('index');
// });
// Use these Routes for Agency users (Owner, Guide, Agent) and Travelers
app.use("/api/users", usersRoutes);
app.use("/api/channels", channelsRoutes);
app.use("/api/tours", toursRoutes);
app.use("/api/agencies", agenciesRoutes);


// Use This for Admin user (administrator) requests
app.use("/api/admin", adminRoutes);

app.get("/uploads/:file_name", async (req, res) => {
  try {
    const fileName = req.params.file_name;
    const data = await getObjectFromS3(fileName);

    res.writeHead(200, { "Content-Type": data.ContentType });
    res.end(data.Body);
  } catch (error) {
    console.error("Error retrieving file:", error);
    res.status(500).json({ error: "Internal Server Error" });
  }
});

// =================================================================================================
// =========================================== Mobile App ROUTES ===================================[]
// =================================================================================================
// Mobile Client routes
app.use("/api/v1/auth", authRoutes);
// app.use('api/v1/channels', touristChannelsRoutes);
app.use("/api/v1/tours", touristToursRoutes);
app.use("/api/v1/requests", bookingRequestRoutes);
app.use("/api/v1/favorite", favoriteRoutes);
app.use("/api/v1/channels", touristChannelRoutes);
app.use("/api/v1/main", mainRoutes);
// Guide Routes
app.use("/api/v2/guide", guideRoutesV2);

// =================================================================================================
// =========================================== Admin's ROUTES =====================================[]
// =================================================================================================

//userProfile Page Routes : /profile
app.use("/api/v2/user-profile", userProfileRoutesAdminPanel);
app.use("/api/v2/agencies", agenciesRoutesV2);
app.use("/api/v2/tours", toursRoutesV2);
app.use("/api/v2/packages", packagesRoutes);
app.use("/api/v2/subscriptions", subscriptionsRoutes);
app.use("/api/v2/payments", paymentsRoutes);

// =================================================================================================
// =========================================== Maroko Ekspert ROUTES ==============================[]
// =================================================================================================

app.use("/api/v3/maroko-ekspert", marokoEkspertRoutes);

// =================================================================================================
// =================================== AGORA CHANNEL GENERATE TOKEN ===============================[]
// =================================================================================================

// Agora Token Generator
app.get("/token", (req, res) => {
  const appId = process.env.APP_KEY;
  const appCertificate = process.env.APP_CERTIFICATE;

  const channelName = req.query.channelName || "demoChannel";
  const uid = req.query.uid || 0;
  const role = RtcRole.PUBLISHER;

  const expirationTimeInSeconds = 3600;

  const currentTimestamp = Math.floor(Date.now() / 1000);

  const privilegeExpiredTs = currentTimestamp + expirationTimeInSeconds;

  const key = RtcTokenBuilder.buildTokenWithUid(
    appId,
    appCertificate,
    channelName,
    uid,
    role,
    privilegeExpiredTs
  );

  res.json({ key });
});

initializeDailyJobs();

server.listen(4000);